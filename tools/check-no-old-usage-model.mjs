...
// shared taxonomy in src/core/usage/NativeUsageTaxonomy.ts.
...
// 2. The persisted UsageEvent schema/allowlist must not return. The live
// taxonomy file is checked, and the old UsageEvent.ts path remains a same-path
// canary after B03 renamed it.
const TAXONOMY_FILES = ['src/core/usage/NativeUsageTaxonomy.ts', 'src/core/usage/UsageEvent.ts'];
for (const rel of TAXONOMY_FILES) {
  ...
  for (const token of ['UsageEventSchema', 'PERSISTED_FIELD_ALLOWLIST', 'USAGE_SOURCE_KINDS']) {
    ...
    if (new RegExp(`export\\s+(const|type)\\s+${token}\\b`).test(src)) {
      violations.push(`persisted usage schema export re-appeared in ${rel}: ${token}`);
    }
  }
}
