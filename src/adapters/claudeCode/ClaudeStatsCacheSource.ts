...
// NATIVE structured per-model cost/model source.
//
// stats-cache.json is a local machine-readable file Claude Code maintains. The
// FILE carries per-model token buckets (input/output/cache), costUSD,
// contextWindow, and maxOutputTokens; the strict per-row schema parses them,
// but the candidates EMITTED below carry only the model id and provider-
// reported cost — v1 renders no token counts. It does NOT carry session/5h or
...
//
// Stats-cache candidates use the distinct native tier `stats_cache_snapshot` —
// NEVER `statusline_snapshot`: a stale stats-cache value must never
...
// Read + parse the stats-cache file at `filePath`, producing per-model
// cost/model cockpit candidates. Fails CLOSED: any read/parse/size failure
