 ...
 * - `export let outputChannel` - read by diagnostics and tests; module-scope intentional
 * - `export let configService` - read by tests via the test API return value; module-scope intentional
 * - `export let secretManager` - read by tests; SecretManager owns SecretStorage access
 * - `export let lastActivationMs` - set at end of activate(); read by test API getter
 *
 * DO NOT strip these exports while fixing lint errors. Exported bindings count
 * as used by the module system.
 *
 * TEST API RETURN VALUE: activate() returns
 * `{ configService, secretManager, saltReady, globalStoragePath, getLastActivationMs }`.
 * Plan 04 + Plan 05 integration tests consume this through the VS Code-loaded
 * extension instance.
 */
...
export let configService: ConfigService | undefined;
export let secretManager: SecretManager | undefined;
export let lastActivationMs: number | undefined;
...
export function activate(context: vscode.ExtensionContext): TokenGaugeTestApi {
