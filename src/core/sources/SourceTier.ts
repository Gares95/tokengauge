...
// (the honesty lattice in src/core/accuracy/Accuracy.ts). Every displayed
// cockpit field carries BOTH a single canonical `sourceTier` AND an
// `accuracyLabel`. SourceTier never replaces, mutates, or feeds
// `Accuracy.combine()` / `leastAccurate()` — the lattice stays untouched.
//
