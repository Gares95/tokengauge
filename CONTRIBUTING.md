...
- **Accuracy labels stay honest.** Labels come from the cockpit resolver's per-source mapping (`SourcePriorityResolver`); never present a value derived from a `proxy_reported` surface as `exact` or `billing_authoritative`.
