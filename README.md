...
- **Honest by construction.** Every number is tracked with an accuracy label (`exact`, `billing_authoritative`, `proxy_reported`, `partial`, `unknown`); cards surface it as plain-language provenance, and Cockpit Diagnostics exposes the raw labels. Missing cost reads `cost unknown`, never a believable `$0.00`.
...
| Codex API-key or different bucket shapes | May be unavailable or partial; TokenGauge does not guess or show Codex cost. |
...
setup, unavailable, or partial states by default. Live usage-window gauges require
a configured Claude statusLine snapshot and/or the explicit Codex probe opt-in.
The local Claude `stats-cache.json` token-detail file is read only when the
Claude card is visible, as described under [Sources](#sources).
...
file, `~/.claude/stats-cache.json` (Claude Code's own token-detail cache; see
[Sources](#sources)).
...
const out = join(homedir(), ".tokengauge", "claude", "statusline-snapshot.json");
  ...
  rate_limits: {
    five_hour: data?.rate_limits?.five_hour
      ? {
          ...
          resets_at_iso: data.rate_limits.five_hour.resets_at_iso,
        }
      : undefined,
    seven_day: data?.rate_limits?.seven_day
      ? {
          ...
          resets_at_iso: data.rate_limits.seven_day.resets_at_iso,
        }
      : undefined,
  },
  context_window: {
...
const fiveHourText = typeof fiveHour === "number" ? `${Math.round(fiveHour)}%` : "--";
console.log(`${model} | 5h ${fiveHourText}`);
...
2. **Edit Claude Code's settings file**: add or update this top-level block in
   ...
   ```json
   {
     "statusLine": {
       "type": "command",
       "command": "node /home/YOUR_USER/.tokengauge/claude/claude-statusline-writer.mjs"
     }
   }
   ```
   ...
   ```json
   {
     "statusLine": {
       "type": "command",
       "command": "node C:/Users/YOUR_USER/.tokengauge/claude/claude-statusline-writer.mjs"
     }
   }
   ```
...
uses the absolute path to `claude-statusline-writer.mjs`.
...
card can go Live; until then it shows an honest waiting or unavailable state. For
exactly which fields are read and how the strict schema rejects anything else, see
[the Claude statusLine bridge setup guide](src/bridge/README-bridge-setup.md)
(in the source repository; it is not packaged in the VSIX).
...
- **Directory mode writes one file per session** and is the recommended choice
  ...
  `~/.tokengauge/claude/statusline-snapshots/` instead of a file.
- The source repo ships a fuller **directory-mode** writer template at
  [`src/bridge/claude-statusline-writer.example.sh`](src/bridge/claude-statusline-writer.example.sh)
  (it writes one hashed file per session). That template lives in the source /
  GitHub repository and is **not installed with the VSIX**. VSIX-only users can
  start with the inline writer above.
...
- **Claude native snapshots.** The cockpit reads a **passive local statusLine snapshot**
  that your own Claude statusLine script writes (an opt-in native bridge), plus the
  local `stats-cache.json` token-detail file.
...
`~/.claude/stats-cache.json` file is Claude Code's own token-detail and cost
cache; TokenGauge reads it whenever the Claude card is visible, it never
supplies the 5h/weekly windows, and if it is missing or unreadable the cockpit
simply shows no token detail.
...
[ACCURACY.md](ACCURACY.md), [SECURITY.md](SECURITY.md),
[CONTRIBUTING.md](CONTRIBUTING.md), and the bridge setup guide live in the
source repository and open from there. Before Marketplace or Open VSX
publication, either package linked docs in the VSIX or convert README links to
absolute hosted repository URLs.
...
**Timing.** The cockpit re-checks snapshot files every 10 to 15 seconds and also
reacts to file changes as they happen. The Codex probe runs at most once per 60
...
- Native snapshot and token-detail files are expected under the **remote**
...
- Path handling is portable across macOS, Linux, and Windows; `~` and relative
  paths are resolved against the host the extension actually runs on.
...
- Writer identity comes from the hashed `session_id_hash`/`workspace_hash`
  fields your writer emits; the writers in this guide emit them. A writer that
...
- The fix at the source is a **per-workspace snapshot file** so sessions in
  different workspaces never clobber each other. The bridge setup guide (in the
  source repository) includes a ready-to-use per-workspace writer template; see
  [the Claude statusLine bridge setup guide](src/bridge/README-bridge-setup.md).
