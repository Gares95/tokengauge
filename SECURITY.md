...
Every production dependency is exact-pinned. The `package-lock.json` is committed. CI installs via `npm ci`. Third-party GitHub Actions are pinned to stable major-version tags in verification workflows; commit-SHA pinning is reserved for the future release workflow.
